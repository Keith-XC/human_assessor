...
def get_100_digit(path, initial_idx=0):
    idx = initial_idx
    ...
    for digit in range(10):
        ...
        select_digits = np.random.choice(files_digit, size=10)
...
def plot_200_digit():
    ...
    big_image_xai = np.zeros((280, 280))
    for i in range(10):
        for j in range(10):
            index = i * 10 + j
    ...
    big_image_random = np.zeros((280, 280))
    for i in range(10):
        for j in range(10):
            index = i * 10 + j + 100
            img = np.load(df['path'][index]).reshape(28,28)
...
def mixed_img():
    ...
    idx_mix_order = np.random.permutation(200)
    print(idx_mix_order)
    df = pd.read_csv("log.csv")
...
"""if os.path.exists("log.csv"):
    os.remove("log.csv")
xai_all = get_100_digit(path_c, initial_idx=0)
random_all = get_100_digit(path_r, initial_idx=100)
plot_200_digit()"""
